...
string rep_with_upper(string str){
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<> distr(0,str.length());
    int randomIndex = distr(gen);


    for(int i =0;i<str.length();i++){
        if(str[i]==str[randomIndex]){
            str[i]= toupper(str[i]);
        }
    }
    return str;
...
string swap_letters(string str){
   ...
   string firstTwo= reinterpret_cast<const char *>(str[0] + str[1]);
   string lastTwo= reinterpret_cast<const char *>(str[str.length()-1] + str[str.length()]);
...
int main() {
    ...
    string initialPassword= passGen(user_number);
    cout<<"Initial password: "<< initialPassword<<endl;

    string finalPassword=swap_letters(rep_with_upper(initialPassword));
    cout<<"Final version of the password: "<< finalPassword<<endl;

    search_letter(passGen(user_number),'s');
